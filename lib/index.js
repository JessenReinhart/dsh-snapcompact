...
export class SnapcompactEngine extends CompactionEngine {
	static inject = ["attachments", "sessions", "tokenMeter"];

	constructor(ctx, config = {}) {
		super(ctx);
		try {
			ctx.provide("snapcompact", this);
		} catch {}
	...
	async compactRegion(start, end, agent, signal, options = {}) {
		...
		// Determine turn owner for session persistence assertion
		let owner = null;
		if (options.owner !== undefined) {
			owner = options.owner;
		} else {
			const lastTurnStart = session.log.findLast(e => e.type === "turn/start");
			const lastTurnEnd = session.log.findLast(e => e.type === "turn/end");
			owner = (lastTurnStart && (!lastTurnEnd || lastTurnEnd.seq < lastTurnStart.seq)) ? lastTurnStart.data.turn : null;
		}

		// Calculate precise shadowedTokenCount from tokenMeter.measure(session)
		const meter = this.ctx.tokenMeter;
		...
		try {
			...
			if (!shadowedTokenCount) {
				shadowedTokenCount = Math.round(transcript.length / 3.5);
			}
			...
			const summaryEvent = session.append("compaction/summary", {
				compactionId,
				...(options.sourceCommandId ? { sourceCommandId: options.sourceCommandId } : {}),
				summary: [{ type: "text", text: `Snapcompact archived ${normalized.length.toLocaleString()} characters into ${imageRefs.length} bitmap frames (~${shadowedTokenCount.toLocaleString()} tokens freed).` }],
		...
		ctx.effect(() => {
			return ctx.commands.register({
				...
				async handler(invocation) {
					...
					try {
						// Pass undefined sourceCommandId so DSH chat UI's compactionDefinition
						// recognizes the checkpoint and renders the native CompactionNodeView card
						const result = await engine.compactNow(agent, invocation.signal, undefined, {
							maxFrames,
							shape,
							retainTokens
						});
